...
public class DefaultJeepOrderDao implements JeepOrderDao {
  ...
  public List<Option> fetchOptions(List<String> optionIds) {
    if (optionIds.isEmpty()) {
      return new LinkedList<>();                                                        // If no options, return empty list
    }

    Map<String, Object> params = new HashMap<>();                                       // Map for list of option_ids
    ...
    for (int index = 0; index < optionIds.size(); index++) {                            // Loop for each option based on size of optionIds list
      String key = "option_" + index;                                                   // Indexes each option in list
      sql += ":" + key + ", ";                                                          // Adds value into map with corresponding key
      params.put(key, optionIds.get(index));
    }

    sql = sql.substring(0, sql.length() - 2);                                           // Removes ", " from each value
    sql += ")";                                                                         // Closing ) for SQL statement IN

    return jdbcTemplate.query(sql, params, new RowMapper<Option>() {
      @Override
      public Option mapRow(ResultSet rs, int rowNum) throws SQLException {              // RowMapper to loop through each option and build object with values
        // @formatter:off
